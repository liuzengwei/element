...
当需要让不同长度的标签在视觉上对齐时，可以使用两端对齐功能。

:::demo 通过设置 `label-justify` 属性为 `true` 可以让标签文本两端对齐。注意：此功能仅在标签文本为单行时生效，如果标签文本过长导致换行，将不会应用两端对齐效果。
```html
<el-row :gutter="20">
  <el-col :span="12">
    <div style="text-align: center; margin-bottom: 10px;">默认对齐</div>
  ...
  <el-col :span="12">
    <div style="text-align: center; margin-bottom: 10px;">两端对齐</div>
    <el-form :model="form" label-width="100px" :label-justify="true">
      <el-form-item label="用户名">
        <el-input v-model="form.name"></el-input>
      </el-form-item>
      <el-form-item label="密码">
...
| label-justify | 表单域标签是否两端对齐，仅在标签文本为单行时生效。如果标签文本超出宽度导致换行，则不应用两端对齐 | boolean | — | false |
...
| label-position<version-badge version="2.15.5-xn.31" type="feature"/> | 表单域标签的位置，如果设置了该属性，会覆盖 Form 组件的 label-position 设置 | string |  right/left/top            | 继承 Form 组件的设置 |
