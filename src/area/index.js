...
export function useCascaderAreaData() {
  ...
  Object.keys(province).forEach((code) => {
    provinceMap[code.slice(0, 2)] = makeOption(province[code], code, []);
  });
  ...
  Object.keys(city).forEach((code) => {
    ...
    cityMap[code.slice(0, 4)] = option;

    const province = provinceMap[code.slice(0, 2)];
    if (province) {
  ...
  Object.keys(county).forEach((code) => {
    const city = cityMap[code.slice(0, 4)];
    if (city) {
      city.children.push(makeOption(county[code], code));
    }
  });

  return Object.keys(provinceMap).map(key => provinceMap[key]);
}
